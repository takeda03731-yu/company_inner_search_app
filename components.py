...
def display_app_title():
    ...
    タイトル表示
    """
    st.markdown(f"## {ct.APP_NAME}", unsafe_allow_html=True)


def display_select_mode():
    """
    回答モードのラジオボタンと説明をサイドバーに表示
    """
    with st.sidebar:
        st.markdown("### 利用目的")
        ...
        # 「社内文書検索」説明
        st.markdown(
            "<div style='font-size:11px; font-weight:bold; background:#e5e5e5; border-radius:6px; padding:6px 10px; margin-bottom:6px;'>【「社内文書検索」を選択した場合】</div>",
            unsafe_allow_html=True
        )
        st.markdown(
            "<div style='font-size:12px; background:#e6f0fa; border-radius:6px; padding:10px 12px; margin-bottom:6px; color:#2563a6;'>入力内容と関連性が高い社内文書のありかを検索できます。</div>",
            unsafe_allow_html=True
        )
        st.markdown(
            "<div style='font-size:11px; background:#f7f7fa; border-radius:6px; padding:8px 12px; margin-bottom:12px; color:#444;'><b>【入力例】</b><br>社員の育成方針に関するMTGの議事録</div>",
            unsafe_allow_html=True
        )

        # 「社内問い合わせ」説明
        st.markdown(
            "<div style='font-size:11px; font-weight:bold; background:#e5e5e5; border-radius:6px; padding:6px 10px; margin-bottom:6px;'>【「社内問い合わせ」を選択した場合】</div>",
            unsafe_allow_html=True
        )
        st.markdown(
            "<div style='font-size:12px; background:#e6f0fa; border-radius:6px; padding:10px 12px; margin-bottom:6px; color:#2563a6;'>質問・要望に対して、社内文書の情報をもとに回答を得られます。</div>",
            unsafe_allow_html=True
        )
        st.markdown(
            "<div style='font-size:11px; background:#f7f7fa; border-radius:6px; padding:8px 12px; color:#444;'><b>【入力例】</b><br>人事部に所属している従業員情報を一覧化して</div>",
            unsafe_allow_html=True
        )

def display_initial_ai_message():
    """
    AIメッセージの初期表示（メイン画面に案内と注意）
    """
    with st.chat_message("assistant"): # 「st.success()」とすると緑枠で表示される 
...
def display_search_llm_response(llm_response):
    ...
    """
    # RAGの履歴をデバッグ表示
    # st.write("RAG Context:", llm_response["context"])
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            # デバッグコードを追加
            # st.write("Metadata:", llm_response["context"][0].metadata)
            # st.write("Main file path:", main_file_path)
            # st.write("Main page number:", llm_response["context"][0].metadata["page"])
            # ページ番号を取得
            main_page_number = llm_response["context"][0].metadata["page"]
            # ページ番号がインデックスなので1を加えて補正
            main_page_number += 1
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                # 参照元のありかに応じて、適したアイコンを取得
                icon = utils.get_source_icon(sub_choice['source'])
                # ページ番号が取得できる場合、PDFのみページ番号を表示
                if "page_number" in sub_choice and sub_choice['source'].lower().endswith(".pdf"):
                    # ぺージ番号を所得
                    corrected_page_number = sub_choice["page_number"]
                    # ページ番号がインデックスなので1を加えて補正
                    corrected_page_number += 1
                    #デバックコードを追加
                    # st.write("Debug - Page Number:", corrected_page_number)
                    #metadataの内容を確認
                    # st.write("Debug - Metadata:", document.metadata)
                    # 「サブドキュメントのファイルパス」と「ページ番号」を表示
                    st.info(f"{sub_choice['source']} (ページNo.{corrected_page_number})", icon=icon)
                else:
                    # 「サブドキュメントのファイルパス」のみを表示
                    st.info(f"{sub_choice['source']}", icon=icon)
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                ...
                # st.write("Page number:", document.metadata["page"])
                # st.write("Total pages:", document.metadata["total_pages"])  # 総ページ数を表示
                # ページ番号を取得
